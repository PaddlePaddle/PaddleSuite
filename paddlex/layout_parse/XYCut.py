...
def split_projection_profile(arr_values: np.array, min_value: float, min_gap: float):
    ...
    ```
         ...
         arr_values           │  │       ┌─┐───
             ┌──┐             │  │       │ │ |
             │  │             │  │ ┌───┐ │ │min_value # 谷值阈值。（高阈值） 还有一个min_gap阈值，用来过滤过小的间隙(宽阈值）。
             │  │<- min_gap ->│  │ │   │ │ │ |
         ────┴──┴─────────────┴──┴─┴───┴─┴─┴─┴───
...
def recursive_yx_cut(boxes: np.ndarray, indices: List[int], res: List[int]):
    """
    ...
    pos_y = split_projection_profile(y_projection, 0, 1) # 山谷高度小于0，宽大于1
    if not pos_y:
        return
...
def recursive_xy_cut(boxes: np.ndarray, indices: List[int], res: List[int]):
    """
    先进行x轴投影，再进行y轴投影的递归切割函数。

    Args:
        boxes: (N, 4) 形状的数组，表示矩形框的 [x_min, y_min, x_max, y_max]。
        indices: 矩形框在原始数据中的索引列表。
        res: 用于存储满足条件的矩形框索引的列表。
    """
    # 向 x 轴投影
    assert len(boxes) == len(indices)

    _indices = boxes[:, 0].argsort()  # 按 x_min 排序
    x_sorted_boxes = boxes[_indices]
    x_sorted_indices = indices[_indices]

    x_projection = projection_by_bboxes(boxes=x_sorted_boxes, axis=0)  # 沿 x 轴投影
    pos_x = split_projection_profile(x_projection, 0, 1)  # 根据投影分割 x 轴区间
    if not pos_x:
        return

    arr_x0, arr_x1 = pos_x
    for c0, c1 in zip(arr_x0, arr_x1):
        # [c0, c1] 表示按照垂直切分有 bbox 的区域，对这些区域会再进行水平切分
        ...
        pos_y = split_projection_profile(y_projection, 0, 4)
        if not pos_y:
            continue

        arr_y0, arr_y1 = pos_y
        if len(arr_y0) == 1:
            # y 方向无法切分，将当前索引添加到结果中
        ...
        for r0, r1 in zip(arr_y0, arr_y1):
            ...
            recursive_xy_cut(
                y_sorted_boxes_chunk[_indices], y_sorted_indices_chunk[_indices], res
            )
