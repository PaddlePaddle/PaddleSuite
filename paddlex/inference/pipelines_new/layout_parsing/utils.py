...
def _get_sub_category( blocks, title_labels):
    ...
    for i, block1 in enumerate(blocks):
        ...
        for j, block2 in enumerate(blocks):
            ...
            match_block_iou = _get_projection_iou(bbox2, bbox1, is_horizontal_1)
            ...
            if block1['label'] in sub_title_labels:
                match_block_iou = _calculate_overlap_area_2_minbox_area_ratio(bbox2, bbox1)
        ...
        def get_sub_category_(title_text_direction,title_text_index,label,is_left_up=True):
            ...
            if title_text_direction == is_horizontal_1 and title_text_index != -1 and (label == "text" or label == "paragraph_title"):
                ...
                if is_horizontal_1:
                    ...
                    if label == "text" :
                        if _nearest_edge_distance(bbox1,bbox2)[0] <= 15 and block1['label'] in vision_labels and \
...
def get_layout_ordering(data,block_index=-1,no_mask_labels=[]):
    ...
    parsing_result,projection_direction = _get_layout_property(parsing_result,
                                                                    median_width,
        ...
        for block in input_blocks:
            ...
            for match_block in parsing_result:
                ...
                if distance_type=="nearest_iou_edge_distance":
                    distance,min_distance_config = _nearest_iou_edge_distance(bbox, 
                                                            match_bbox,
                                                            block['sub_label'],
                    ...
                    if match_block['label'] in title_labels+["abstract"] and match_block['title_text'] != []:
                        iou_left_up = _calculate_overlap_area_2_minbox_area_ratio(bbox,match_block['title_text'][0][1])
                        iou_right_down = _calculate_overlap_area_2_minbox_area_ratio(bbox,match_block['title_text'][-1][1])
                    ...
                    if match_block['label'] in vision_labels and match_block['vision_footnote'] != []:
                        iou_left_up = _calculate_overlap_area_2_minbox_area_ratio(bbox,match_block['vision_footnote'][0])
                        iou_right_down = _calculate_overlap_area_2_minbox_area_ratio(bbox,match_block['vision_footnote'][-1])
                        iou = 1-max(iou_left_up,iou_right_down)
                        distance = _manhattan_distance(bbox, match_bbox)*iou
                    else:
                        distance = float('inf')
                    ...
                    if match_block['label'] in vision_title_labels and block['vision_footnote'] != []:
                        iou_left_up = _calculate_overlap_area_2_minbox_area_ratio(match_bbox,block['vision_footnote'][0])
                        iou_right_down = _calculate_overlap_area_2_minbox_area_ratio(match_bbox,block['vision_footnote'][-1])
                        iou = 1-max(iou_left_up,iou_right_down)
                        distance = _manhattan_distance(bbox, match_bbox)*iou
                    else:
                        distance = float('inf')
...
def _calculate_horizontal_distance( input_bbox, match_bbox, height, disperse, title_text):
    """
...
def _nearest_edge_distance( 
    ...
    """
    match_bbox_iou = _calculate_overlap_area_2_minbox_area_ratio(input_bbox, match_bbox)
...
def _nearest_iou_edge_distance( 
    ...
    if horizontal1 != horizontal2 or _get_projection_iou(input_bbox, match_bbox, horizontal1) < 0.01:
        iou_distance = 1
        ...
        height = y2 - y1
        if horizontal1:
            return _calculate_horizontal_distance(input_bbox, match_bbox, height, disperse, title_text), \
                    min_distance_config
        else:
            return _calculate_vertical_distance(input_bbox, match_bbox, width, disperse, title_text), \
                    min_distance_config
    ...
    if label == "abstract":
        tolerance_len *= 3
    edge_distance, edge_distance_config = _nearest_edge_distance(input_bbox, match_bbox, weight, 
                                                                    label=label, 
                                                                    no_mask_labels=no_mask_labels, 
